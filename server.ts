...
async function startServer() {
  ...
  app.use((req, res, next) => {
    if (!req.cookies['proxy-session']) {
      const newSessionId = uuidv4();
      res.cookie('proxy-session', newSessionId, { 
        httpOnly: true, 
        sameSite: 'none', 
        secure: true,
      ...
      req.cookies['proxy-session'] = newSessionId;
    }
    next();
  });
        ...
        (function() {
          ...
          window.addEventListener('click', (e) => {
            ...
            if (link && link.href && !link.href.startsWith('javascript:')) {
              if (!link.href.includes(proxyBase)) {
                e.preventDefault();
          ...
          window.addEventListener('submit', (e) => {
            ...
            if (form.action && !form.action.includes(proxyBase)) {
              form.action = proxyUrl(form.getAttribute('action'));
            }
          }, true);

          // Handle window.location assignments (best effort)
          const originalLocation = window.location;
          // Note: window.location is mostly read-only/special, but we can try to catch some things
  ...
  app.all("/api/proxy", async (req, res) => {
    ...
    const sessionId = req.cookies['proxy-session'];
    ...
    try {
      ...
      if (setCookie) {
        ...
        if (Array.isArray(setCookie)) {
          await Promise.all(setCookie.map(cookie => jar.setCookie(cookie, targetUrl)));
        } else {
          await jar.setCookie(setCookie, targetUrl);
        }
      }
